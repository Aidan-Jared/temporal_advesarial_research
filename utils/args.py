...
def get_single_arg_value(
    parser: ArgumentParser, arg_name: str, cmd: Optional[List[str]] = None
):
    """
...
def fix_model_parser_backwards_compatibility(
    main_parser: ArgumentParser, model_parser: ArgumentParser = None
) -> ArgumentParser:
    """
...
def build_parsable_args(parser: ArgumentParser, spec: dict) -> None:
    ...
    for name, arg_spec in spec.items():
        ...
        if any([action.dest == name for action in parser._actions]):
            logging.warning(
                f"Argument `{name}` is already defined in the parser. Skipping..."
            )
            continue
...
def add_dynamic_parsable_args(
    parser: ArgumentParser, dataset: str, backbone: str
) -> None:
    """
    ...
    ds_group = parser.add_argument_group(
        "Dataset arguments", "Arguments used to define the dataset."
    )
    registered_datasets = get_dataset_names()
    if isinstance(dataset, dict):
        assert "type" in dataset, (
            "The dataset `type` (i.e., the registered name) must be defined in the dictionary."
        )
        bk_name = dataset["type"].replace("_", "-").lower()
        bk_args = {**registered_datasets[bk_name]["parsable_args"], **dataset["args"]}
        dataset = bk_name
    else:
        bk_args = registered_datasets[dataset.replace("_", "-").lower()][
            "parsable_args"
        ]
    build_parsable_args(ds_group, bk_args)

    bk_group = parser.add_argument_group(
        "Backbone arguments", "Arguments used to define the backbone network."
    )
    if isinstance(backbone, dict):
        assert "type" in backbone, (
            "The backbone `type` (i.e., the registered name) must be defined in the dictionary."
        )
        bk_name = backbone["type"].replace("-", "_").lower()
        bk_args = {**REGISTERED_BACKBONES[bk_name]["parsable_args"], **backbone["args"]}
        backbone = bk_name
    else:
        bk_args = REGISTERED_BACKBONES[backbone.replace("_", "-").lower()][
            "parsable_args"
        ]
...
def add_configuration_args(parser: ArgumentParser, args: Namespace) -> None:
    ...
    config_group = parser.add_argument_group(
        ...
        "Arguments used to define the dataset and model configurations.",
    )
...
def add_initial_args(parser, strict=True) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=list(REGISTERED_BACKBONES.keys()),
        default="resnet18",
    )
    parser.add_argument(
        "--load_best_args",
...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    exp_group = parser.add_argument_group(
        "Experiment arguments", "Arguments used to define the experiment settings."
    )
    ...
    exp_group.add_argument(
        ...
        required=True,
        type=float,
        help="Learning rate. This should either be set as default by the model "
    ...
    noise_group = parser.add_argument_group(
        "Noise arguments", "Arguments used to define the noisy-label settings."
    )

    noise_group.add_argument(
        "--noise_type",
        type=field_with_aliases(
            {
    ...
    noise_group.add_argument(
        "--noise_rate", type=float, default=0, help="Noise rate in [0-1]."
    )
    noise_group.add_argument(
...
def add_poison_args(parser: ArgumentParser) -> None:
    ...
    Returns:
        None
    """
    poison_group = parser.add_argument_group(
        "Poison arguments",
    ...
    poison_group.add_argument(
        ...
        type=float,
        default=None,
        help="The percentage of poisoned classes in a task, if not provided will poison half the classes.",
    )
...
def add_management_args(parser: ArgumentParser) -> None:
    ...
    Returns:
        None
    """
    mng_group = parser.add_argument_group(
    ...
    mng_group.add_argument(
        "--disable_log", default=0, type=binary_to_boolean_type, help="Disable logging?"
    )
    mng_group.add_argument(
        "--num_workers",
        type=int,
    ...
    mng_group.add_argument(
        "--ckpt_name", type=str, help="(optional) checkpoint save name."
    )
    mng_group.add_argument(
        "--start_from", type=int, default=None, help="Task to start from"
    )
    mng_group.add_argument("--stop_after", type=int, default=None, help="Task limit")
    mng_group.add_argument(
        "--save_after_interrupt",
        type=binary_to_boolean_type,
        default=1,
    ...
    wandb_group = parser.add_argument_group(
        "Wandb arguments", "Arguments to manage logging on Wandb."
    )
...
def add_rehearsal_args(parser: ArgumentParser) -> None:
    ...
    group = parser.add_argument_group(
        "Rehearsal arguments", "Arguments shared by all rehearsal-based methods."
    )
...
class _DocsArgs:
    ...
    def parse_choices(self) -> str:
        if self.choices is None:
            return ""
        return ", ".join(
            [c.keys() if isinstance(c, dict) else str(c) for c in self.choices]
        )
...
def _parse_actions(actions: list, group_name: str, group_desc: str) -> _DocArgsGroup:
    ...
    for action in actions:
        ...
        docs_args.append(
            _DocsArgs(
                action.dest, action.type, action.choices, action.default, action.help
            )
        )
...
if __name__ == "__main__":
    ...
    for group in parser._action_groups:
        if len([a for a in group._group_actions if a.dest != "help"]) == 0:
            continue
        docs_args.append(
            _parse_actions(group._group_actions, group.title, group.description)
        )
