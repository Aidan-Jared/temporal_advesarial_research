...
EvalFn = Callable[["ContinualModel", "ContinualDataset", bool, bool], Any]


@torch.no_grad()
def evaluate(
    model: "ContinualModel", dataset: "ContinualDataset", last=False, return_loss=False
) -> Tuple[list, list]:
    """
    ...
    tot_seen_samples = 0
    total_len = (
        sum(len(x) for x in dataset.test_loaders)
        if hasattr(dataset.test_loaders[0], "__len__")
        else None
    )
    ...
    pbar = tqdm(
        ...
        total=total_len,
        desc="Evaluating",
        disable=model.args.non_verbose,
    )
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        while True:
            ...
            if (
                "class-il" not in model.COMPATIBILITY
                and "general-continual" not in model.COMPATIBILITY
            ):
            ...
            i += 1
            pbar.set_postfix(
                {f"acc_task_{k + 1}": max(0, correct / total * 100)}, refresh=False
            )
            ...
            if dataset.SETTING == "class-il":
                ...
                eces_masked_task += calibration_error(
                        outputs[:, dataset.get_offsets(k)[0] : dataset.get_offsets(k)[1]],
                        labels,
                        n_bins=15,
                        task="multiclass",
                        num_classes=dataset.get_offsets(k)[1] - dataset.get_offsets(k)[0],
                    ).item()
        ...
        if correct > correct_mask_classes and dataset.SETTING == "class-il":
            logging.warning(
                "Task-IL accuracy is LOWER than Class-IL accuracy. "
        ...
        accs.append(
            correct / total * 100
            if "class-il" in model.COMPATIBILITY
            or "general-continual" in model.COMPATIBILITY
            else 0
        )
        eces.append(
            eces_task / total
        ...
        masked_eces.append(eces_masked_task / total)
    pbar.close()

    model.net.train(status)
    if return_loss:
        return accs, accs_mask_classes, avg_loss / tot_seen_samples
    return accs, accs_mask_classes, eces, masked_eces
